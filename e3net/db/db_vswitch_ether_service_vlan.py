...
def load_ether_service_vlans_from_db():
    ...
    try:
        ...
        session.close()
register_database_load_entrance('ether_service_vlan',load_ether_service_vlans_from_db)

def db_register_vswitch_ether_service_vlan(fields_create_dict):
    session=db_sessions[DB_NAME]()
    try:
        session.begin()
        vlan=session.query(E3EtherServiceVlan).filter(and_(E3EtherServiceVlan.lanzone_id==fields_create_dict['lanzone_id'],
            E3EtherServiceVlan.vlan_id==fields_create_dict['vlan_id'])).first()
        if vlan:
...
def db_get_vswitch_ether_service_vlan(uuid):
    ...
    try:
        session.begin()
        vlan=session.query(E3EtherServiceVlan).filter(E3EtherServiceVlan.id==uuid).first()
        if not vlan:
