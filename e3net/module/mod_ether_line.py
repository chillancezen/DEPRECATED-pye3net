...
def _do_creating_topology(config,iResult):
    ...
    while True:
        ...
        for _lanzone_id in intermediate_lanzone:
            ...
            if _neighbor_weight>(_prev_weight+edge_weight):
                temporary_lanzone_set[_lanzone_id]=(_prev_weight+edge_weight,(iface0_id,host_id,iface1_id))
    ...
    for lanzone_id in permanent_lanzone_set:
        ...
        iface0,host,iface1=path
        e3loger.debug('\tlanzone_id:%s'%(lanzone_id)+':<weight:%d,<iface0:%s,host:%s,iface1:%s>>'%(weight,iface0,host,iface1))
