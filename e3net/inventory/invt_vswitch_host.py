...
def invt_update_vswitch_host(host_uuid,fields_change_dict,user_sync=False,user_timeout=default_user_timeout):
    base=get_inventory_base()
    assert(base)
    sub_key=host_uuid
    base.update_object(root_key,sub_key,fields_change_dict,user_sync=user_sync,user_timeout=user_timeout)

def invt_unregister_vswitch_host(host_uuid,user_sync=False,user_timeout=default_user_timeout):
    sub_key=host_uuid
    base=get_inventory_base()
    assert(base)
