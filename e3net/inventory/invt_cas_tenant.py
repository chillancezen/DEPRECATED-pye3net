...
def invt_register_cas_tenant(fields_create_dict,user_sync=False,user_timeout=default_user_timeout):
...
def invt_update_cas_tenant(tenant_uuid,fields_change_dict,user_sync=False,user_timeout=default_user_timeout):
    base=get_inventory_base()
    assert(base)
    sub_key=tenant_uuid
    base.update_object(root_key,sub_key,fields_change_dict,user_sync=user_sync,user_timeout=user_timeout)

def invt_unregister_cas_tenant(tenant_uuid,user_sync=False,user_timeout=default_user_timeout):
    sub_key=tenant_uuid
    base=get_inventory_base()
    assert(base)
